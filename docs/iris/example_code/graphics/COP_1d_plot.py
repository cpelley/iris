...
----------
   ...
   Johns T.C., et al. (2010) Climate change under aggressive mitigation:
       The ENSEMBLES multi-model
   experiment. Climate Dynamics (submitted)

   Lowe J.A., C.D. Hewitt, D.P. Van Vuuren, T.C. Johns, E. Stehfest,
       J-F. Royer, and P. van der Linden, 2009.
   New Study For Climate Modeling, Analyses, and Scenarios. Eos Trans. AGU,
       Vol 90, No. 21.
...
def main():
    ...
    # Take an area-weighted collapse.
    pre_industrial_mean = pre_industrial.weighted_collapsed(
        ['latitude', 'longitude'], iris.analysis.MEAN)
    e1_mean = e1.weighted_collapsed(
        ['latitude', 'longitude'], iris.analysis.MEAN)
    a1b_mean = a1b.weighted_collapsed(
        ['latitude', 'longitude'], iris.analysis.MEAN)
