...
class LinearInterpolator(object):
    def __init__(self, cube, interp_coords, extrapolation_mode=_MODE_LINEAR):
        self.cube = cube
        self.coord_points = []
        self.coord_dims = []
        self.ndims = None
    ...
    def _setup(self):
        ...
        # Triggers the loading - is that really necessary...?
        data = self.cube.data
        self.ndims = len(self._interp_coords)
        self.coords = [self.cube.coord(coord) for coord in self._interp_coords]
    ...
    def _interpolate(self, data, coord_points):
        ...
        if self._interpolator is None:
            self._interpolator = _RegularGridInterpolator(self.coord_points,
                                                          data,
                                                          bounds_error=False,
                                                          fill_value=None)
        ...
        * interpolated_data - The ``data`` array interpolated using the given
        ...
        if coord_points.shape[-1] != self.ndims:
            msg = 'The given coordinates are not appropriate for' \
                ...
                ' have a shape of (npts, {0}). Got an array of' \
                ' shape {1}.'
            raise ValueError(msg.format(self.ndims,
                                        np.asanyarray(coord_points).shape))
